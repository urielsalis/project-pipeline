...
class New: CliktCommand(help="Create new pipeline from template") {
    ...
    override fun run() {
        ...
        with(URL(jenkinsHost+"/createItem?name=$name").openConnection() as HttpURLConnection) {
            ...
            addRequestProperty("Authorization", Base64.getEncoder().encode(jenkinsUserPass.toByteArray(Charsets.UTF_8)).toString(Charsets.UTF_8))
            val wr = OutputStreamWriter(outputStream)
            wr.write("""
                <?xml version='1.<flow-definition plugin="workflow-job@2.24">
                        <actions>
                    ...
                    </flow-definition>1' encoding='UTF-8'?>
            """.trimIndent())
            wr.flush()
